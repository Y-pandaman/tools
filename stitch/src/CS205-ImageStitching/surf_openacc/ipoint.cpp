...
cv::Mat getWarppedAcc(IplImage *original, cv::Mat H)
{
    ...
    uchar *warp_data = new uchar[h*w*2*channel]; // height, wdith, channel
    memset(warp_data, 0, h*w*2*channel);

    int warpStep = 2*step;

    #pragma acc data copyin(src[0:h*w*channel], h, w,\
         channel, step, warpStep, H_[0:9]) \
         copy(warp_data[0:w*2*h*channel])
    {
...
std::vector<cv::Mat> getWarppedAcc_blend(IplImage *original, cv::Mat H)
{
    double H_[9] = {
        H.at<double>(0, 0), H.at<double>(0, 1), H.at<double>(0, 2), 
        H.at<double>(1, 0), H.at<double>(1, 1), H.at<double>(1, 2), 
    ...
    #pragma acc data copyin(src[0:h*w*channel], h, w,\
         channel, step, warpStep, maskStep, H_[0:9]) copy(warp_data[0:w*2*h*channel], mask_data[0:w*2*h])
    {
    #pragma acc parallel loop //private(ii, jj, fx, fy, cx, cy, x, y, z, r, g, b)
    for(int i = (int)(-h/1.2); i < h * 1.2; ++i)
    {
        // int ii = std::min(h-1, std::max(i, 0));
        int ii = i > (h-1) ? h-1 : i<0 ? 0 : i;

        #pragma acc loop //private(jj, fx, fy, cx, cy, x, y, z, r, g, b)
