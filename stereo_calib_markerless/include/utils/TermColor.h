...
public:
  ...
  static std::string compose(const int modifier, const int color,
                             bool invert = false) {
    ...
    if (!invert)
      ...
      buffer << "\033[" << CTRL_INVERSE << ";" << modifier << ";" << color
             << "m";
    return buffer.str();
  }
  ...
  static const int CTRL_INVERSE_OFF =
      27; //(swap foreground and background colours)
};
