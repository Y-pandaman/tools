...
MonoGeometry::MonoGeometry(camodocal::CameraPtr _camera) {
  assert(_camera && "Abstract Camera is not set. You need to init camera "
                    "before setting it to geometry clases");
  if (!_camera) {
    cout << "Abstract Camera is not set. You need to init camera before "
            "setting it to geometry clases\n";
    exit(10);
  }
  this->camera = _camera;
  ...
  this->camera->initUndistortRectifyMap(map_x, map_y, new_fx, new_fy,
                                        cv::Size(0, 0), new_cx, new_cy);
}
...
void MonoGeometry::do_image_undistortion(const cv::Mat &im_raw,
...
StereoGeometry::StereoGeometry(camodocal::CameraPtr _left_camera,
  ...
  if (!_left_camera || !_right_camera) {
    cout << "\nERROR : Abstract stereo Camera is not set. You need to init "
            "camera before setting it to geometry clases\n";
    exit(10);
  }
  assert(_left_camera && _right_camera &&
         "Abstract stereo Camera is not set. You need to init camera before "
         "setting it to geometry clases");

  this->camera_left = _left_camera;
  this->camera_right = _right_camera;
  // this->right_T_left = Matrix4d( __right_T_left ); //stereo extrinsic.
  // relative pose between two pairs.
  this->set_stereoextrinsic(
      __right_T_left); // stereo extrinsic. relative pose between two pairs.
  ...
  right_geom->set_K(
      this->K_new); // it is really important that you have same K_new for each
                    // of the images in the stereo-pair, this is not a mistake.

  // stereo rectification maps
  // theory :
  // http://homepages.inf.ed.ac.uk/rbf/CVonline/LOCAL_COPIES/FUSIELLO/tutorial.html
  // make_stereo_rectification_maps() // no need to call this again as it is
  // already called when u either do set_K() or set_stereoextrinsic().

  // init SGBM
  // TODO Can put in options to the BM algorithm, look at opencv docs to set the
  // options like blocksize, numDisparities etc etc.
...
void StereoGeometry::fundamentalmatrix_from_stereoextrinsic(Matrix3d &F) {
  ...
  F = this->get_K().transpose().inverse() * Tx * _Rot *
      this->get_K().inverse(); //< Fundamental Matrix
}

void StereoGeometry::draw_epipolarlines(cv::Mat &imleft_undistorted,
                                        cv::Mat &imright_undistorted) {
  ...
  cout << "[StereoGeometry::draw_epipolarlines]F=" << F.format(numpyFmt)
       << endl;

  // tryout multiple points and get their corresponding epipolar line.
  for (int i = 0; i < 500; i += 20) {
#if 1
    // take a sample point x on left image and find the corresponding line on
    // the right image
...
#if 1
    // take a sample point on right image and find the corresponding line on the
    // left image
    Vector3d xd(i, i, 1.0);
...
void StereoGeometry::draw_srectified_epipolarlines(
  ...
  Matrix3d F = rm_fundamental_matrix; //< This was computed by
                                      //make_stereo_rectification_maps.
  // this->fundamentalmatrix_from_stereoextrinsic( F );
  cout << "[StereoGeometry::draw_srectified_epipolarlines]F=\n"
       << F.format(numpyFmt) << endl;

  // tryout multiple points and get their corresponding epipolar line.
  cv::RNG rng;
  for (int i = 0; i < imleft_srectified.rows; i += 20) {
    ...
    Vector3d x(1.5 * i, i, 1.0);
    Vector3d ld = F * x;
    MiscUtils::draw_point(x, imleft_srectified, color);
    MiscUtils::draw_line(ld, imright_srectified, color);
#endif
    ...
    Vector3d xd(i, i, 1.0);
    Vector3d l = F.transpose() * xd;
    MiscUtils::draw_line(l, imleft_srectified, color);
    MiscUtils::draw_point(xd, imright_srectified, color);
#endif
  }
}
...
void StereoGeometry::set_K(float _fx, float _fy, float _cx, float _cy) {
  std::lock_guard<std::mutex> lk(m_intrinsics);
  this->K_new << _fx, 0.0, _cx, 0.0, _fy, _cy, 0.0, 0.0, 1.0;
...
void StereoGeometry::make_stereo_rectification_maps() {
  cv::Size imsize =
      cv::Size(camera_left->imageWidth(), camera_left->imageHeight());

  IOFormat numpyFmt(FullPrecision, 0, ", ", ",\n", "[", "]", "[", "]");

  // Adopted from :
  // http://homepages.inf.ed.ac.uk/rbf/CVonline/LOCAL_COPIES/FUSIELLO/node18.html
  ...
  __StereoGeometry___make_stereo_rectification_maps(
      ...
      // Matrix3d K_new_eigen = get_K(); //< this causes some issues. Better not
      // use it.
      Matrix3d K_new_eigen = this->K_new; // get_K();
  __StereoGeometry___make_stereo_rectification_maps(
      cout << "K_new_eigen: " << K_new_eigen << endl;) cv::Mat K_new_cvmat;
  ...
  cv::Mat D;
  // !! Main Call OpenCV !! //
  /// The below function does exactly this:
  /// http://homepages.inf.ed.ac.uk/rbf/CVonline/LOCAL_COPIES/FUSIELLO/node18.html
  // See OpenCVdoc:
  // https://docs.opencv.org/3.4/d9/d0c/group__calib3d.html#ga617b1685d4059c6040827800e72ad2b6
  cv::stereoRectify(K_new_cvmat, D, K_new_cvmat, D, imsize, R, T, rm_R1, rm_R2,
                    rm_P1, rm_P2, rm_Q);

  __StereoGeometry___make_stereo_rectification_maps(
      cout << "\tK_new=" << cv::format(K_new_cvmat, cv::Formatter::FMT_NUMPY)
           << endl;
      ...
      // P2(0,3) := Tx * f, where f = P2(0,0) and Tx is the position of left
      // camera as seen by the right camera. right_T_left.
      double Tx = rm_P2.at<double>(0, 3) / rm_P2.at<double>(0, 0);
  rm_shift = Vector3d(Tx, 0, 0);
  __StereoGeometry___make_stereo_rectification_maps(
      cout << "\tTx = " << Tx
           << " Tx is the position of left camera as seen by the right camera. "
              "right_T_left"
           << endl);
  Matrix3d TTx;
  ...
  // P2(1,3) := Ty * f, where f = P2(0,0) and Tx is the horizontal shift between
  // cameras double Ty = P2(1,3) / P2(0,0);
  ...
  cv::initUndistortRectifyMap(K_new_cvmat, D, rm_R1, rm_P1, imsize, CV_32FC1,
                              map1_x, map1_y);
  cv::initUndistortRectifyMap(K_new_cvmat, D, rm_R2, rm_P2, imsize, CV_32FC1,
                              map2_x, map2_y);
  __StereoGeometry___make_stereo_rectification_maps(
...
void StereoGeometry::do_image_undistortion(const cv::Mat &imleft_raw,
                                           const cv::Mat &imright_raw,
                                           cv::Mat &imleft_undistorted,
                                           cv::Mat &imright_undistorted) {
  std::lock_guard<std::mutex> lk(m_intrinsics);
...
void StereoGeometry::do_stereo_rectification_of_undistorted_images(
    ...
    cv::Mat &imleft_srectified, cv::Mat &imright_srectified) {
  cv::remap(imleft_undistorted, imleft_srectified, this->map1_x, this->map1_y,
            CV_INTER_LINEAR);
  cv::remap(imright_undistorted, imright_srectified, this->map2_x, this->map2_y,
            CV_INTER_LINEAR);
}
...
void StereoGeometry::do_stereo_rectification_of_raw_images(
  ...
  // raw --> undistorted
  do_image_undistortion(imleft_raw, imright_raw, imleft_undistorted,
                        imright_undistorted);

  // undistorted --> stereo rectify
  do_stereo_rectification_of_undistorted_images(
...
// ie. cv::StereoBM
// if you use this function, besure to call
// `do_stereo_rectification_of_undistorted_images` or
// `do_stereo_rectification_of_raw_images` on your images before calling this
// function
void StereoGeometry::do_stereoblockmatching_of_srectified_images(
    ...
    cv::Mat &disparity) {
  // sgbm->compute( imleft_srectified, imright_srectified, disparity );
  // //disparity is CV_16UC1
  bm->compute(imleft_srectified, imright_srectified,
              disparity); // disparity is CV_16UC1
  // cout << "[StereoGeometry::do_stereoblockmatching_of_srectified_images]
  // disparity" << MiscUtils::cvmat_info( disparity ) << endl;
}
...
void StereoGeometry::do_stereoblockmatching_of_raw_images(
    const cv::Mat &imleft_raw, const cv::Mat &imright_raw, cv::Mat &disparity) {
  cv::Mat imleft_srectified, imright_srectified;
  do_stereo_rectification_of_raw_images(imleft_raw, imright_raw,
                                        imleft_srectified, imright_srectified);
  do_stereoblockmatching_of_srectified_images(imleft_srectified,
                                              imright_srectified, disparity);
}
...
//  fill_eigen_matrix : _3dpts will be allocated and filled only if this flag is
//  true. If this flag is false then eigen matrix is not filled in this
//  function. make_homogeneous : true will result in Eigen matrix being 4xN else
//  will be 3xN. fill_eigen_matrix need to be true for this to matter.
// # Outputs
//  out3d : 3 channel image. X,Y,Z --> ch0, ch1, ch2. Will also contain invalid
//  3d points. _3dpts : 4xN matrix containing only valid points. N <
//  disparity_raw.shape[0]*disparity_raw.shape[1].
void StereoGeometry::disparity_to_3DPoints(const cv::Mat &disparity_raw,
                                           cv::Mat &out3D, MatrixXd &_3dpts,
                                           bool fill_eigen_matrix,
  ...
  for (int i = 0; i < disparity.rows; i++) {
    ...
    for (int j = 0; j < disparity.cols; j++) {
      const float pw = 1.0f / (disp_ptr[j] / 16. * Q32 + Q33 +
                               1E-6); // 1E-6 added to avoid inf.

      // ---------------------------------^^^^^
      // it is crucial that you convert the raw_disparity to
      // CV_32F and divide by 16. This is because raw_disparity from StereoBM()
      // is in CV_16SC1 (16bit) and a disparity value := NumDisparities*16. so
      // if you have NumDisparities as 64 the maximum value in disparity will be
      // 1024. This is a bit weird. So be careful.
  ...
  // if using opencv's reprojectImageTo3D() it is crucial that you convert the
  raw_disparity to
  // CV_32F and divide by 16. This is because raw_disparity from StereoBM() is
  in CV_16SC1
  // (16bit) and a disparity value := NumDisparities*16. so if you have
  NumDisparities as 64
  // the maximum value in disparity will be 1024. This is a bit weird. So be
  careful.
  ...
  */
  ...
  // TODO: Make this more efficient. If I enable 4xN matrix it takes 8ms/image
  // if I disable it takes 3ms/image. Step-1: loop over out3D to know how how
  // many were valid points. remove inf points and remove behind points (points
  // where disparity was invalid)
  int N = 0;
  for (int i = 0; i < disparity.rows; i++) {
    const float *disp_ptr = disparity.ptr<float>(i);
    cv::Vec3f *out3D_ptr = out3D.ptr<cv::Vec3f>(i);
...
void StereoGeometry::get3dpoints_from_raw_images(const cv::Mat &imleft_raw,
                                                 const cv::Mat &imright_raw,
                                                 MatrixXd &_3dpts) {
  cv::Mat disp_raw;
...
void StereoGeometry::get3dpoints_and_3dmap_from_raw_images(
...
void StereoGeometry::get3dpoints_and_3dmap_from_raw_images(
    const cv::Mat &imleft_raw, const cv::Mat &imright_raw, MatrixXd &_3dpts,
    cv::Mat &_3dImage, cv::Mat &imleft_srectified,
    cv::Mat &imright_srectified) {
  // raw --> srectified
  this->do_stereo_rectification_of_raw_images(
      imleft_raw, imright_raw, imleft_srectified, imright_srectified);

  // srectified --> disp
  cv::Mat disp_raw;
  this->do_stereoblockmatching_of_srectified_images(
      imleft_srectified, imright_srectified, disp_raw);

  const cv::Mat Q = this->get_Q();
  this->disparity_to_3DPoints(disp_raw, _3dImage, _3dpts, true, true);
}

void StereoGeometry::get3dpoints_and_disparity_from_raw_images(
    const cv::Mat &imleft_raw, const cv::Mat &imright_raw, MatrixXd &_3dpts,
  ...
  cv::normalize(disp_raw, disparity_for_visualization_gray, 0, 255, CV_MINMAX,
                CV_8U); //< disp8 used just for visualization
  cv::applyColorMap(disparity_for_visualization_gray,
                    disparity_for_visualization, cv::COLORMAP_HOT);
...
void StereoGeometry::
    get_srectifiedim_and_3dpoints_and_disparity_from_raw_images(
        const cv::Mat &imleft_raw, const cv::Mat &imright_raw,
        cv::Mat &imleft_srectified, cv::Mat &imright_srectified,
...
void StereoGeometry::
    get_srectifiedim_and_3dpoints_and_3dmap_and_disparity_from_raw_images(
        const cv::Mat &imleft_raw, const cv::Mat &imright_raw,
        cv::Mat &imleft_srectified, cv::Mat &imright_srectified,
...
// _3dImage: 3d points as 3 channel image. 1st channel is X, 2nd channel is Y
// and 3rd channel is Z. Also note that these co-ordinates and imleft_raw
// co-ordinate do not correspond. They correspond to the srectified images.
// Incase you want to use the color info with these 3d points, this will lead to
// wrong. You should compute the srectified images for that.
//      1. raw --> srectified
//      2. srectified --> disparity_raw
//      3. disparity_raw --> 3d points
void StereoGeometry::get3dmap_from_raw_images(const cv::Mat &imleft_raw,
                                              const cv::Mat &imright_raw,
                                              cv::Mat &_3dImage) {
  cv::Mat disp_raw;
  this->do_stereoblockmatching_of_raw_images(imleft_raw, imright_raw, disp_raw);
...
// e_3dImageX, e_3dImageY, e_3dImageZ: cv::split( _3dImage ). same size as
// imleft_raw.shape. Also note that these co-ordinates and imleft_raw
// co-ordinate do not correspond. They correspond to the srectified images.
// Incase you want to use the color info with these 3d points, this will lead to
// wrong. You should compute the srectified images for that.
//      1. raw --> srectified
...
void StereoGeometry::get3dmap_from_raw_images(const cv::Mat &imleft_raw,
  ...
  cout << "this is somewhat fragile. I am making a 3 array of matrix and "
          "returning it. This is bad. this need to be tested for correctness "
          "after this can be used safely.";
  exit(11);
...
//-------------------------------------------------------------------------------------//
void GeometryUtils::make_K(float new_fx, float new_fy, float new_cx,
                           float new_cy, Matrix3d &K) {
  K << new_fx, 0, new_cx, 0, new_fy, new_cy, 0, 0, 1;
}

void GeometryUtils::getK(camodocal::CameraPtr m_cam, Matrix3d &K) {
  Matrix3d K_rect;
  vector<double> m_camera_params;
  m_cam->writeParameters(
      m_camera_params); // retrive camera params from Abstract Camera.
  // camodocal::CataCamera::Parameters p();
  // cout << "size=" << m_camera_params.size() << " ::>\n" ;
  // for( int i=0 ; i<m_camera_params.size() ; i++ ) cout << "\t" << i << " " <<
  // m_camera_params[i] << endl;

  switch (m_cam->modelType()) {
  case camodocal::Camera::ModelType::MEI:
    K_rect << m_camera_params[5], 0, m_camera_params[7], 0, m_camera_params[6],
        m_camera_params[8], 0, 0, 1;
    break;
  case camodocal::Camera::ModelType::PINHOLE:
    K_rect << m_camera_params[4], 0, m_camera_params[6], 0, m_camera_params[5],
        m_camera_params[7], 0, 0, 1;
    break;
  case camodocal::Camera::ModelType::KANNALA_BRANDT:
    ...
    break;
  default:
    // TODO: Implement for other models. Look at initUndistortRectifyMap for
    // each of the abstract class.
    cout << "[getK] Wrong\nQuit....";
    cout << "Implement for other models. Look at initUndistortRectifyMap for "
            "each of the abstract class.\n";
    exit(10);
  }
  K = Matrix3d(K_rect);
...
// given a point cloud as 3xN or 4xN matrix, gets colors for each based on the
// depth
void GeometryUtils::depthColors(const MatrixXd &ptcld,
                                vector<cv::Scalar> &out_colors, double min,
                                double max) {
  assert((ptcld.rows() == 3 || ptcld.rows() == 4) &&
         "ptcld need to be either a 3xN or a 4xN matrix");
  assert(ptcld.cols() > 0);
...
// given a point cloud as 3xN or 4xN matrix, gets colors for each based on the
// depth. return in out_colors as 3xN
void GeometryUtils::depthColors(const MatrixXd &ptcld, MatrixXd &out_colors,
